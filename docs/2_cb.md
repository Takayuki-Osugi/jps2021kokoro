...
- 変化の見落としはなぜ起こるのでしょうか？
- ブランク画面（空白画面）を表示せずに２枚の画像を交互に表示すると，変化箇所は簡単に見つけることができます。変化箇所が動いて見えるためです。
- このことから２つのことが分かります。
...
以下の２つの条件を比べてみてください。
 <a href="./image/noblank.gif" target="_blank">ブランク画面（空白画面）なし条件</a>
 <a href="./image/blank.gif" target="_blank">ブランク画面（空白画面）あり条件</a>
...
- 過渡的な信号を消すためにブランク画面（空白画面）を用いました。
- 独立変数はブランク画面（空白画面）の有無（あり条件，なし条件）でした。
- 従属変数として変化を検出するまでの反応時間を測定しました。
<img src='./image/RT.jpeg' width='40%'>

### **それでは以下のリンクをクリックして実験を体験してみましょう**
