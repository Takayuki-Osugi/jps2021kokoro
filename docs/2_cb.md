...
<img src='./image/stim.jpeg' width='100%'>
...
```note
#### 変化の見落としはフリッカー法以外でも起こるの？
映画で場面（シーン）が切り替わるような状況で変化をさせたり，現実場面で遮蔽物の陰で変化をさせたりした場合でも変化の見落としが起こることが知られています。
```

## 2.4. 結果を見てみよう

条件ごとに反応時間の平均を算出しました。尚，正解した試行のみを集計しています。以下はクラウドソーシングサービスで募集をした実験の結果です。右側がスマートフォンで参加したグループで，左側がPCで参加したグループです。ブランク画面なし条件とあり条件の結果が示されています。

<img src='./image/result.jpeg' width='100%'>
...
<img src='./image/RT2.jpeg' width='80%'>
...
<img src='./image/RT.jpeg' width='80%'>


### 2.6.2. 独立変数，従属変数，剰余変数について
...
#### 独立変数とは？
実験や調査では，実施者側があらかじめパラメータを何種類か用意しておく場合があります。例えば，今回の場合は，「ブランク画面あり条件」と「ブランク画面なし条件」を用意しました。このように実施者側で操作する変数のことを独立変数といいます。
...
<img src='./image/within.jpeg' width='80%'>

参加者間計画（対応なし）

<img src='./image/between.jpeg' width='80%'>
...
### 2.6.3. 順序効果とカウンターバランスについて

今回は常にブランク画面なし条件→あり条件の順番で実験を実施しました。そのため，順序効果が結果に影響を及ぼしている可能性があります。

順序効果とは課題を行った順序が結果に影響を及ぼすことです。練習効果，疲労効果，残留効果などが知られています。
- 練習効果とは，前半に行った試行よりも後半に行った試行で，パフォーマンスが良くなる効果です。
- 疲労効果とは，前半に行った試行よりも後半に行った試行で，パフォーマンスが悪くなる効果です。
- 残留効果とは，先に行った課題の影響が次に行った課題に影響することです。


<img src='./image/practice.jpeg' width='80%'>

順序効果によって結果が歪むのを防ぐためには，２つの条件を行う順番を，ある参加者は「条件1→条件2」の順番，別の参加者は「条件2→条件1」の順番にするなどして，統制することが重要です。この方法のことをカウンターバランスという言い方をします。
...
- 熊田孝恒 2012 マジックにだまされるのはなぜか 「注意」の認知心理学 化学同人
- 横澤一彦 2010 視覚科学 勁草書房
- 横澤一彦 2017 つじつまを合わせたがる脳 岩波書店
